...
Se uma informação não existir, retorne null ou string vazia.`
    ...
    if (!base64Data) {
      return new Response(JSON.stringify({ error: 'O arquivo PDF base64 está vazio ou inválido.' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      })
    }
    ...
    if (!response.ok) {
      ...
      return new Response(JSON.stringify({ error: 'Erro ao processar o documento. Verifique o arquivo e tente novamente.' }), {
        status: 502,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
